...
def data_sort(metric):
    ...
    if metric == "cells":
        for file_ in file_list:
            try:
                data = pl.scan_csv(f"{dir_}_results_{metric}/{file_}.csv".select(["Area", "Minor", "Major"])

                count = data.select("Area").collect().height # Number of cells
                mean_area = data.select("Area").mean().collect()
                mean_width = data.select("Minor").mean().collect()
                mean_length = data.select("Major").mean().collect()
